...
from apps.authentication.util import generate_token, verify_pass
...
def login():
    ...
    if flask.request.method == "POST":
        ...
        login_response = invoke_lambda_function(
            "dynamodb-lambda-checkloginlambdafunction77404F11-KArjitXGmsrv",
            {"username": username, "password": password},
        )
        login_result = json.loads(login_response["Payload"].read().decode())
        body_content = json.loads(login_result["body"])
        if (
            login_response["StatusCode"] != 200
            or body_content["login_successful"] is False
        ):
...
def register():
    ...
    if "register" in request.form:
        ...
        duplicate_response = invoke_lambda_function(
            "dynamodb-lambda-checkduplicateusernamelambdafuncti-abhSKTg7U0CT",
            {"username": username},
        )
        duplicate_result = json.loads(
            duplicate_response["Payload"].read().decode()
        )
        ...
        add_user_response = invoke_lambda_function(
            "dynamodb-lambda-writetodynamodblambdafunction902BB-gKNxlXgscQ1X",
            {"username": username, "password": request.form["password"]},
        )
        add_user_result = json.loads(
            add_user_response["Payload"].read().decode()
        )
